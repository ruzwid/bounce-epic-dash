...
const STATUS_TO_BASIS_KEY: Record<SubtaskStatus, keyof Omit<ScoreBasis, "total">> = {
  shipped: "shipped",
  staged: "staged",
  in_review: "inReview",
...
export function computeScore(
  ...
  weights: Record<SubtaskStatus, number>,
  ...
  const weightedSum = subtaskStatuses.reduce((sum, status) => sum + weights[status], 0);
  const mean = weightedSum / subtaskStatuses.length;
 ...
 *  100 done — and "done" additionally requires every subtask shipped to
 ...
 *  subtask actually shipped is "nearly_done", never "done". */
export function deriveStage(score: number, allSubtasksShippedToDefault: boolean): Stage {
  if (score === 0) return "not_started";
  if (score < 25) return "early";
  if (score < 70) return "underway";
  if (score < 100) return "nearly_done";
  return allSubtasksShippedToDefault ? "done" : "nearly_done";
}
